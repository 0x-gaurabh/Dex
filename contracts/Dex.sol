...
import "./interface/IERC20.sol";
 ...
 import "@openzeppelin/contracts/utils/ReentrancyGuard.sol";
    ...
    contract Dex is ReentrancyGuard {
        ...
        function getUserBalance(address _token,address _user) public view returns(uint256) {
       ...
       function getAmountOut(uint256 _amountIn, uint256 _reserveIn, uint256 _reserveOut) public  pure returns(uint256 _amountOut){
        uint256 amountInWithFee = (_amountIn * 997) / 1000;

        _amountOut= (_reserveOut * amountInWithFee) /
    ...
    require(IERC20(_tokenOut).transfer(msg.sender, _amountOut), "Transfer failed");

    return _amountOut;
}
        function addLiquidity(uint256 _amount0,uint256 _amount1) external returns (uint256 shares) {

            require(_amount0>0 && _amount1>0,"Amount should be greater than zero");
